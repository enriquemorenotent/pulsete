...
test('startup repair backfills the message search index when schema drift hides it on latest versions', () => {
  const dir = mkdtempSync(join(tmpdir(), 'pulsete-storage-'));
  const file = join(dir, 'db.sqlite');
  const existing = new DatabaseSync(file);
  const now = Date.now();
  ...
  const indexCount = repaired.prepare('SELECT COUNT(*) AS count FROM messages_fts').get() as { count: number };
  repaired.close();
