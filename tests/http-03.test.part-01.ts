...
test('muted nick routes persist entries, recompute unread, and broadcast updates without auth', async () => {
  ...
  try {
    const mutedPromise = waitForWebSocketMessageType(socket, 'muted-nick.upsert');
    ...
    const createResponse = await requestJson(port, 'POST', '/api/muted-nicks', {
      networkId: network.id,
      nick: 'alice',
    });
    assert.equal(createResponse.status, 200);
    ...
    const duplicateResponse = await requestJson(port, 'POST', '/api/muted-nicks', {
      networkId: network.id,
      nick: 'ALICE',
    });
    assert.equal(duplicateResponse.status, 200);
    ...
    const removePromise = waitForWebSocketMessageType(socket, 'muted-nick.remove');
    ...
    const deleteResponse = await requestJson(port, 'DELETE', `/api/muted-nicks/${mutedMessage.mutedNick.id}`);
    assert.equal(deleteResponse.status, 200);
    assert.equal(deleteResponse.json.ok, true);
...
test('muted nick routes validate payloads, networks, and targets', async () => {
  ...
  try {
    const invalidPayload = await requestJson(port, 'POST', '/api/muted-nicks', { nick: {} });
    assert.equal(invalidPayload.status, 400);
    assert.equal(invalidPayload.json.message, 'Invalid muted nick payload');

    const missingNetwork = await requestJson(port, 'POST', '/api/muted-nicks', {
    ...
    const invalidTarget = await requestJson(port, 'POST', '/api/muted-nicks', {
      networkId: network.id,
      nick: '#help',
    });
    assert.equal(invalidTarget.status, 400);
    ...
    const reservedTarget = await requestJson(port, 'POST', '/api/muted-nicks', {
      networkId: network.id,
      nick: 'Server',
    });
    assert.equal(reservedTarget.status, 400);
    ...
    const multipleTargets = await requestJson(port, 'POST', '/api/muted-nicks', {
      networkId: network.id,
      nick: 'alice,bob',
    });
    assert.equal(multipleTargets.status, 400);
    ...
    const missingDelete = await requestJson(port, 'DELETE', '/api/muted-nicks/missing-muted-nick');
    assert.equal(missingDelete.status, 404);
