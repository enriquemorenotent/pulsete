...
test('runtime clears cached friend presence when a watched peer quits', async () => {
  ...
  storage.conversations.upsertChannel({ networkId: network.id, name: '#help' });
  const friend = runtime.friends.upsertFriend('Alice').friend;
  ...
  try {
    ...
    const connection = runtime.connections.get(network.id);
    assert.ok(connection);
    connection.consume(':tester!user@host JOIN #help\r\n');
    connection.consume(':Alice!user@host JOIN #help\r\n');
    connection.consume(':Alice!user@host QUIT :bye\r\n');
